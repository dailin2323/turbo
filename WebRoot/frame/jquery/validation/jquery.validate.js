...
(function($) {
	...
	$.extend($.fn, {
		...
		validate : function(options) {
			...
			if (validator.settings.onsubmit) {
				...
				this.validateDelegate(":submit", "click", function(event) {
							...
							// allow suppressing validation by adding a cancel
		...
		rules : function(command, argument) {
			...
			data = $.validator.normalizeRules($.extend({}, $.validator
									.classRules(element), $.validator
									.attributeRules(element), $.validator
									.dataRules(element), $.validator
	...
	$.extend($.validator, {
		...
		prototype : {
			...
			init : function() {
				...
				function delegate(event) {
					var validator = $.data(this[0].form, "validator"), eventType = "on"
							+ event.type.replace(/^validate/, ""), settings = validator.settings;
				...
				// Add aria-required to any Static/Data/Class required fields
				// before first validation
				// Screen readers require this attribute to be present before
			...
			resetForm : function() {
				...
				this.elements().removeClass(this.settings.errorClass)
						.removeData("previousValue").removeAttr("aria-invalid");
			},
			...
			check : function(element) {
				...
				var rules = $(element).rules(), rulesCount = $.map(rules,
						function(n, i) {
							return i;
				...
				for (method in rules) {
					...
					try {
						...
						result = $.validator.methods[method].call(this, val,
			...
			// return the generic message if present and no method specific
			// message is present
		...
		attributeRules : function(element) {
			...
			for (method in $.validator.methods) {
				...
				// convert the value to a number for number inputs, and for text
		...
		dataRules : function(element) {
			...
			for (method in $.validator.methods) {
				value = $element.data("rule" + method[0].toUpperCase()
						+ method.substring(1).toLowerCase());
				if (value !== undefined) {
					rules[method] = value;
				}
